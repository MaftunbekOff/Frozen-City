...
| `sim/` (modul-ildiz `sim.rs` + submodullar: `mapgen`·`text`·`players`·`command`·`tick`, V0.18'dan `expedition`·`lifecycle`·`lawbook`·`missions`) | Determinstik state-mashina: xarita generatsiyasi, tick mantig'i, buyruq qo'llash | `new_game(seed, win_days)` (shaxsiy/umumiy olam), `new_game_central(seed)` (markaziy olam — aholisiz, missiyasiz), `tick(state)` (har 200ms: ochlik/harorat/ishlab-chiqarish/voqealar — `central` bo'lsa ochlik/o'lim/g'alaba/voqealar o'tkazib yuboriladi), `apply_command(state, player, cmd)` (`can_issue`ni tekshirib bajaradi), `extract_migrants`/`inject_migrants` (Tunnel orqali ko'chish) va **`extract_settlers`/`inject_returnees`** (V0.18: markazdan uyga qaytish), `player_joined_as`/`player_rejoined`/`kick_player` | `fc-net`'s `server.rs` (`sim_loop`), `client/local_server.rs` (wasm inline), barcha `tests/*.rs` |
| ↳ V0.19 `snow` submoduli | Qor yog'ishi, Qor brigadasi va qo'lda tozalash | `snow::{tick_snowfall, tick_snow_crews, tick_clear_orders}`. `Tile.snow` — bayt, ya'ni bir tikdagi ulush butun songa yetmaydi; shu sabab `units_due` yordamida **aniq kunlik tezlik**da qo'llanadi (qat'iy "har N tikda" oralig'i doim biroz ko'p beradi). Qor **hech qachon to'smaydi** — `Tile::speed_factor` nolga tushmaydi, chunki xaritada yo'l-topish yo'q va o'tib bo'lmaydigan tayl odamni ko'rinmas tarzda qamab qo'yardi | `sim/tick.rs` (yurish narxi va `road_step`) |
| ↳ V0.18 submodullari | Har yangi mexanika `tick.rs`ni o'stirmasdan o'z faylida yashaydi; `tick` ularga bittadan chaqiruv nuqtasi bilan kiradi | `expedition::{launch, recall, tick_expeditions}` (guruh safar davomida `survivors`dan butunlay chiqadi), `lifecycle::{starting_age, tick_lifecycle, clear_partner_links}` (yosh/juftlik/qarilik), `lawbook::{enact, repeal}`, `missions::tick_mission_cycles` (takroriy tsikllar). **Qonun effektlari `tick`da `match Law` bilan emas** — `GameState::law_*_multiplier` so'rovlari orqali o'qiladi, shu sabab yangi qonun `types/law.rs`dagi ma'lumot o'zgarishi xolos | `sim/tick.rs`, `sim/command.rs`, `tests/*.rs` |
...
| `world_manager.rs` | Akkaunt → shaxsiy olam marshrutlash, lazy-spawn/idle-evict | `WorldManager::join_account` (login → akkauntning o'z `sim_loop` thread'i, birinchi so'rovda spawn qilinadi), `enter_central` (Tunnel bitirgan akkauntni markaziy olamga kiritadi, `extract_migrants`/`inject_migrants` orqali aholi ko'chiradi, `CENTRAL_MIGRANTS_PER_ACCOUNT=5`gacha), **`return_home`** (V0.18: teskari yo'nalish — markazdagi ko'chmanchilarni olib, shaxsiy olamga qaytaradi; sig'maganini markazga qaytarib qo'yadi, `central_entry` qulfi ikki yo'nalishni bir vaqtda bo'lishiga yo'l qo'ymaydi), idle-evict `IDLE_SHUTDOWN`dan keyin, `MAX_ACCOUNT_WORLDS=200` cap (markaziy olam bundan mustasno) | `main.rs` (`run_dedicated`), `server.rs::route_first_msg` |
| `market.rs` (V0.18, **faqat native**) | Global bozor: o'yinchi-o'yinchi buyurtma kitobi | `handle(account, msg, state)` (`RefreshMarket`/`PostOrder`/`TakeOrder`/`CancelOrder`), `claim_wallet(account, state)`. Buyurtmalar akkauntlar DB'sida (`market_orders`), qo'yilganda tovar/oltin **darhol eskrovga** olinadi. Egasi oflayn bo'lishi mumkinligi uchun daromad `market_wallet` qatoriga tushadi va olamga keyingi kirishda to'lanadi — ishlamayotgan olamga yozish yo'q. Markaziy olamda savdo taqiqlangan (u yerdagi zaxira umumiy) | `server/simloop.rs` (marshrutlash + join'da `claim_wallet`) |
...
| `persist.rs` | Diskka saqlash/yuklash, versiyalangan format | `save_at`/`load_at` (joriy header **`FCWORLD17`** (V0.20) + bincode; atomik yozish: temp fayl + rename). Har eski header o'z `legacy.rs` ko'zgusi orqali o'qilib zanjir bo'ylab migratsiya qilinadi (magic'siz V1 fayllargacha) | `server.rs` (`sim_loop`, `save_world`), `world_manager.rs`, `examples/checksave.rs` |
| `legacy.rs` | Muzlatilgan eski format ko'zgulari — bir marta yozilgach **hech qachon o'zgartirilmaydi** | Har versiya uchun o'z strukturasi (`GameStateV1`…`GameStateV16`, `SurvivorV8`/`V13`/`V14`, `BuildingV12`/`V16`, `TileV15`, `StockpileV7`/`V8`/`V10`) va `impl From<...> for GameState` zanjiri. Yangi maydonga **xavfsiz** default beriladi — masalan V0.18'da `age_days` nolga emas, `migrated_age(id)` orqali ishlaydigan kattalar yoshiga tushadi: nol butun koloniyani bir zumda ishlay olmaydigan bolalarga aylantirar edi | `persist.rs::load_at` |
...
3. **Markaziy olam (Global Olam)** — `EnterCentral` bilan, faqat Tunnel
   ...
   **V0.18 — qaytish yo'li:** `ReturnHome` bilan o'sha ko'chmanchilar
   (`extract_settlers`/`inject_returnees`) shaxsiy olamga qaytariladi —
   kasbi/XP'si bilan birga, `owner` tegi olib tashlanadi (uyda hech kim
   "akkaunt ko'chmanchisi" emas). Markazda qurilgan binolar joyida qoladi:
   uyga qaytish — Global Olamdan chiqib ketish emas, shunchaki safar. Keyin
